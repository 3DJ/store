...
pid = ::File.join(node[:deploy_to], "shared", "pids", "unicorn.pid")
...
node.default[:unicorn][:stderr_path] = ::File.join(node[:deploy_to], "current", "log", "unicorn.log")
node.default[:unicorn][:stdout_path] = ::File.join(node[:deploy_to], "current", "log", "unicorn.log")
